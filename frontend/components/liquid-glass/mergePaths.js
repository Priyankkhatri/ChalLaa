...
export function flattenPath(d) {
  ...
  const cubic = (x1, y1, x2, y2, ex, ey) => {
    ...
    for (let s = 1; s <= BEZIER_STEPS; s++) {
      ...
      const u = 1 - t;
      push(u * u * u * x0 + 3 * u * u * t * x1 + 3 * u * t * t * x2 + t * t * t * ex, u * u * u * y0 + 3 * u * u * t * y1 + 3 * u * t * t * y2 + t * t * t * ey);
    }
    lastC = [x2, y2];
    lastQ = null;
    x = ex;
...
function signedDistance(polys, px, py) {
  let best = Infinity;
  let inside = false;
 ...
 */
function contourSegments(field, gw, gh, sx, sy) {
  const segs = [];
  ...
  for (let gy = 0; gy < gh - 1; gy++) {
    for (let gx = 0; gx < gw - 1; gx++) {
      const i0 = gy * gw + gx;
      const v = [at(field, i0), at(field, i0 + 1), at(field, i0 + gw + 1), at(field, i0 + gw)];
      let mask = 0;
      for (let n = 0; n < 4; n++) if (at(v, n) < 0) mask |= 1 << n;
      if (mask === 0 || mask === 15) continue;
      const p = [[gx * sx, gy * sy], [(gx + 1) * sx, gy * sy], [(gx + 1) * sx, (gy + 1) * sy], [gx * sx, (gy + 1) * sy]];
      const crossings = [];
      for (let n = 0; n < 4; n++) {
        const a = at(v, n);
        const b = at(v, (n + 1) % 4);
 ...
 *   something wrong.
 */
export function mergePathOutline(dA, dB, viewBoxWidth, viewBoxHeight, k) {
  ...
  const key = pt => `${Math.round(pt[0] / eps)},${Math.round(pt[1] / eps)}`;
  ...
  for (let s = 0; s < segs.length; s++) {
    ...
    parts.push(`M ${fmt(at(loop, 0)[0])} ${fmt(at(loop, 0)[1])} ${loop.slice(1).map(pt => `L ${fmt(pt[0])} ${fmt(pt[1])}`).join(' ')} Z`);
  }
