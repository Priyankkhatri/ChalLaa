 ...
 */
const NATIVE_ONLY_PROPS = ['intensity', 'interactive', 'tilt', 'refraction', 'thickness', 'blurRadius', 'specular', 'edgeReflectionStrength', 'legibilityFloor', 'paused',
// Look-shaping uniforms — they exist only inside the AGSL shader, so there is
// nothing honest the fallback can do with them.
'iridescence', 'grain', 'lightAngle', 'specularSharpness', 'saturation', 'brightness', 'magnification', 'ior', 'frameStatsInterval', 'onFrameStats',
 ...
 */
export function renderWebGlass(props) {
  if (__DEV__) {
  ...
  const content = scrim > 0 ? /*#__PURE__*/_jsxs(_Fragment, {
