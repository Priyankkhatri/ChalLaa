...
function circlePath(cx, cy, r) {
  const o = r * KAPPA;
  return [`M ${fmt(cx)} ${fmt(cy - r)}`, `C ${fmt(cx + o)} ${fmt(cy - r)} ${fmt(cx + r)} ${fmt(cy - o)} ${fmt(cx + r)} ${fmt(cy)}`, `C ${fmt(cx + r)} ${fmt(cy + o)} ${fmt(cx + o)} ${fmt(cy + r)} ${fmt(cx)} ${fmt(cy + r)}`, `C ${fmt(cx - o)} ${fmt(cy + r)} ${fmt(cx - r)} ${fmt(cy + o)} ${fmt(cx - r)} ${fmt(cy)}`, `C ${fmt(cx - r)} ${fmt(cy - o)} ${fmt(cx - o)} ${fmt(cy - r)} ${fmt(cx)} ${fmt(cy - r)}`, 'Z'].join(' ');
}
 ...
 */
function polygonShape(sides, rotationDeg, cornerRadius = 0) {
  ...
  const at = i => verts[(i % count + count) % count] ?? [50, 50];
...
function heartShape() {
  ...
  'C 42 12 24 8 13.5 18', 'C 3 28 3.5 45 13 56.5', 'C 22 67.5 38 80 50 92',
  // right lobe, mirrored
  'C 62 80 78 67.5 87 56.5', 'C 96.5 45 97 28 86.5 18', 'C 76 8 58 12 50 26', 'Z'].join(' ');
  return {
    path,
    ...UNIT_BOX
  };
