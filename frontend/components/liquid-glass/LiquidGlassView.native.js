...
import { DEFAULT_RESOLVED_ACCESSIBILITY } from "./accessibility.js";
import { validateGlassProps } from "./devValidate.js";
import { GlassAccessibilityGate } from "./GlassAccessibilityGate.js";
import NativeLiquidGlass from './LiquidGlassmorphismViewNativeComponent';
import { resolvePreset } from "./presets.js";
import { mergePathOutline } from "./mergePaths.js";
 ...
 */
function narrowHandler(handler) {
 ...
 */
const ANDROID_ONLY_KEYS = ['edgeReflectionStrength', 'legibilityFloor', 'paused', 'iridescence', 'grain', 'lightAngle', 'specularSharpness', 'saturation', 'secondaryShape', 'shapeSmoothing', 'brightness', 'magnification', 'ior', 'frameStatsInterval', 'onFrameStats'];
function androidGlassProps(resolved, /** Already normalised to an SVG path — derived, not a raw prop. */
secondaryShapePath, onFrameStats) {
  const {
...
export function LiquidGlassView(props) {
  return /*#__PURE__*/_jsx(GlassAccessibilityGate, {
    props: props,
    renderGlass: renderNativeGlass
  });
}
 ...
 */
export function renderNativeGlass(props, a11y = DEFAULT_RESOLVED_ACCESSIBILITY) {
  ...
  const handlePipelineReady = narrowHandler(onPipelineReady);
  const handleError = narrowHandler(onError);
  ...
  const handleFrameStats = narrowHandler(resolved.onFrameStats);
  ...
  // wrong.
  const iosMerged = !isAndroid && normalized && normalizedSecondary ? mergePathOutline(normalized.path, normalizedSecondary.path, normalized.viewBoxWidth, normalized.viewBoxHeight, shapeSmoothingValue) : null;
  const effectiveShapeProps = iosMerged ? {
    ...shapeProps,
    shapePath: iosMerged
  } : shapeProps;
  ...
  const sharedProps = {
    ...
    blurRadius: typeof blurRadius === 'number' && Number.isFinite(blurRadius) ? Math.max(0, blurRadius) : -1
  };
  ...
  // and ignored.
  const platformProps = isAndroid ? androidGlassProps(resolved, normalizedSecondary?.path ?? '', handleFrameStats) : null;
  const hostRest = {
    ...rest
  };
...
//# sourceMappingURL=LiquidGlassView.native.js.map