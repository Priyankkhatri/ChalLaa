...
export const DEFAULT_ACCESSIBILITY_STATE = Object.freeze({
 ...
 */
export const DEFAULT_RESOLVED_ACCESSIBILITY = Object.freeze({
  opaque: false,
  allowMotion: true
});
 ...
 */
export function resolveAccessibility(mode, state) {
  ...
  switch (mode) {
    ...
    default:
      return {
        opaque: state.reduceTransparency,
        allowMotion
      };
  }
}
 ...
 */
async function readReduceTransparency() {
  try {
...
async function readReduceMotion() {
  try {
...
function subscribe(event, handler) {
  try {
    const sub = AccessibilityInfo.addEventListener(event, handler);
    return () => sub?.remove?.();
  } catch {
    return () => {};
  }
 ...
 */
export function useGlassAccessibilityState() {
  const [state, setState] = useState(DEFAULT_ACCESSIBILITY_STATE);
  useEffect(() => {
    let cancelled = false;
    const refresh = async () => {
      const [reduceTransparency, reduceMotion] = await Promise.all([readReduceTransparency(), readReduceMotion()]);
      if (cancelled) return;
      // Replace only on a real change, so an unrelated AppState churn does not
      // re-render every glass view in the tree.
      setState(prev => prev.reduceTransparency === reduceTransparency && prev.reduceMotion === reduceMotion ? prev : {
        reduceTransparency,
        reduceMotion
      });
    };
    refresh();
    const unsubscribers = [subscribe('reduceTransparencyChanged', reduceTransparency => setState(prev => prev.reduceTransparency === reduceTransparency ? prev : {
      ...prev,
      reduceTransparency
    })), subscribe('highTextContrastChanged', reduceTransparency => setState(prev => Platform.OS !== 'android' || prev.reduceTransparency === reduceTransparency ? prev : {
      ...prev,
      reduceTransparency
    })), subscribe('reduceMotionChanged', reduceMotion => setState(prev => prev.reduceMotion === reduceMotion ? prev : {
      ...prev,
      reduceMotion
    }))];
